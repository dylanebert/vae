...
class OneHot:
    def __init__(self, params, num_classes):
        ...
        x = Input(shape=(image_size, image_size, num_channels))
        y = Input(shape=(num_classes,))

        #latent space Z (mean and std)
        z_mean = Dense(latent_size)(y)
        z_stddev = Dense(latent_size)(y)
        ...
        #Model for full vae
        self.vae = Model([x, y], x_decoded_mean_squash)
...
if __name__ == '__main__':
    ...
    params = Params()
    train_generator = DataGenerator(data_path + 'train/', params, one_hot=True)
    dev_generator = DataGenerator(data_path + 'dev/', params, one_hot=True)
    test_generator = DataGenerator(data_path + 'test/', params, one_hot=True)
    ...
    if args.train:
        ...
        empty = np.zeros(shape=(num_classes, params.image_size, params.image_size, params.num_channels))
        identity = np.identity(num_classes)
        class_means = network.encoder.predict([empty, identity], verbose=1)
