...
for i, (w1, w2) in enumerate(wordnet_gmc_pairs):
    if os.path.exists(os.path.join('/data/nlp/vae/model/gmc/sets', w1 + '_' + w2)):
        continue
            ...
            if os.path.exists(w1_filepath):
                ...
                for col_method in classifier_methods:
                    ...
                    if os.path.exists(w2_filepath):
                        ...
                        if len(w1_members) > 0 and len(w2_members) > 0:
    ...
    for dir in exemplar_subdirs:
        row_header = dir
        exemplar_path = os.path.join(exemplar_dir, dir, w1)
